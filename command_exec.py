...
directory_clean = current_directory + '/cleanpdf'
dataset_a = [['obj', 'endobj', 'stream', 'endstream', 'xref', 'trailer', 'startxref', 'Page', 'Encrypt',
...
directory_malware = current_directory + '/maliciouspdf'
for filename in os.listdir(directory_malware):
    if filename.endswith(""):
        print(filename)
        command_to_execute = 'python3 pdfid.py ' + 'maliciouspdf/' + '"' + filename + '"'
        stdout = Popen(command_to_execute, shell=True, stdout=PIPE).stdout
        output = stdout.readlines()
        if len(output) == 24:
            dataset_a.append(getFeatures(output, 'yes'))
        continue
    else:
...
print("----Clean Files-----")
for filename in os.listdir(directory_clean):
    if filename.endswith(".pdf"):
        print(filename)
        command_to_execute = 'python3 pdfid.py ' + 'cleanpdf/' + '"' + filename + '"'
        stdout = Popen(command_to_execute, shell=True, stdout=PIPE).stdout
        output = stdout.readlines()
        if len(output) == 24:
            dataset_a.append(getFeatures(output, 'no'))
        continue
    else:
        print("Skipped file:", filename)
        continue
